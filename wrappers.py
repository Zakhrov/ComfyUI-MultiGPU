...
def _create_distorch_safetensor_v2_override(
    cls,
    device_param_name,
    device_setter_func,
    apply_device_kwarg_workaround,
    eject_models_default=True,
):
    ...
    class NodeOverrideDisTorchSafetensorV2(cls):
        ...
        def INPUT_TYPES(s):
            ...
            inputs["optional"][device_param_name] = (
                devices,
                {"default": default_device},
            )
            inputs["optional"]["virtual_vram_gb"] = (
                "FLOAT",
            ...
            inputs["optional"]["eject_models"] = (
                "BOOLEAN",
                {"default": eject_models_default},
            )
            return inputs
        ...
        def IS_CHANGED(
            ...
            **kwargs,
            ...
            if not hasattr(cls, "_last_hash"):
                ...
                cls._last_hash = current_hash
                logger.mgpu_mm_log(
                    f"IS_CHANGED CHANGED: {current_hash[:8]} ← settings changed"
                )
            return current_hash
        ...
        def override(
            ...
            **kwargs,
            ...
            original_device = (
                get_current_device()
                if device_param_name == "compute_device"
                else get_current_text_encoder_device()
            )
            ...
            if eject_models:
                logger.mgpu_mm_log(
                    f"[EJECT_MODELS_SETUP] eject_models=True - marking all loaded models for eviction, device target: {device_value}"
                )
                ejection_count = 0
                ...
                logger.mgpu_mm_log(
                    "[EJECT_MODELS_SETUP] eject_models=False - loading without eviction"
                )
            ...
            full_allocation = (
                f"{expert_mode_allocations}#{vram_string}"
...
def override_class_with_distorch_gguf(cls):
    ...
    class NodeOverrideDisTorchGGUFLegacy(cls):
        ...
        def INPUT_TYPES(s):
            ...
            inputs["optional"]["virtual_vram_gb"] = (
                "FLOAT",
                {"default": 4.0, "min": 0.0, "max": 24.0, "step": 0.1},
            )
            ...
            **kwargs,
            ...
            if virtual_vram_gb > 0:
                if use_other_vram:
                    ...
                    other_devices.sort(
                        key=lambda x: int(x.split(":")[1] if ":" in x else x[-1]),
                        reverse=False,
                    )
...
def override_class_with_distorch_gguf_v2(cls):
    ...
    class NodeOverrideDisTorchGGUFv2(cls):
        ...
        def INPUT_TYPES(s):
            ...
            inputs["optional"] = inputs.get("optional", {})
            inputs["optional"]["compute_device"] = (
                devices,
        ...
        def override(self, *args, device=None, **kwargs):
            ...
            try:
                with cuda_device_guard(
                    target_device, reason=f"{type(self).__name__}.{cls.FUNCTION}"
                ):
