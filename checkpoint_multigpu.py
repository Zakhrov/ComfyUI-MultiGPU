...
def patch_load_state_dict_guess_config():
    ...
    if original_load_state_dict_guess_config is not None:
        logger.debug(
            "[MultiGPU Checkpoint] load_state_dict_guess_config is already patched."
        )
        return

    logger.info(
        "[MultiGPU Core Patching] Patching comfy.sd.load_state_dict_guess_config for advanced MultiGPU loading."
    )
...
def patched_load_state_dict_guess_config(
    sd,
    output_vae=True,
    output_clip=True,
    ...
    from . import (
        set_current_device,
        set_current_text_encoder_device,
        get_current_device,
    ...
    try:
        ...
        if model_config is None:
            logger.warning(
                "[MultiGPU] Warning: Not a standard checkpoint file. Trying to load as diffusion model only."
            )
        ...
        logger.debug(
            f"[MultiGPU] Detected Model Config: {type(model_config).__name__}, Parameters: {parameters/10**9:.2f}B"
        )
        ...
        unet_compute_device = torch.device(
            device_config.get("unet_device", original_main_device)
        )
        if model_config.scaled_fp8 is not None:
            manual_cast_dtype = mm.unet_manual_cast(
                None, unet_compute_device, model_config.supported_inference_dtypes
            )
        else:
            manual_cast_dtype = mm.unet_manual_cast(
                unet_dtype, unet_compute_device, model_config.supported_inference_dtypes
            )
        ...
        if model_config.clip_vision_prefix is not None and output_clipvision:
            clipvision = comfy.clip_vision.load_clipvision_from_sd(
        ...
        if output_model:
            ...
            model_patcher = model_patcher_class(
                model,
                load_device=unet_compute_device,
            ...
            logger.mgpu_mm_log(
                "Invoking soft_empty_cache_multigpu before UNet ModelPatcher setup"
            )
            ...
            if distorch_config and "unet_allocation" in distorch_config:
                ...
                if unet_alloc:
                    ...
                    model._distorch_high_precision_loras = distorch_config.get(
                        "high_precision_loras", True
                    )

        if output_vae:
            vae_target_device = torch.device(
                device_config.get("vae_device", original_main_device)
            )
        ...
        if output_clip:
            ...
            clip_target_device = torch.device(
                device_config.get("clip_device", original_clip_device)
            )
            ...
            if clip_target is not None:
                ...
                if len(clip_sd) > 0:
                    logger.debug(
                        "[MultiGPU Checkpoint] Invoking soft_empty_cache_multigpu before CLIP construction"
                    )
                    ...
                    if distorch_config and "clip_allocation" in distorch_config:
                        ...
                        if clip_alloc and hasattr(clip, "patcher"):
                            ...
                            clip.patcher.model._distorch_high_precision_loras = (
                                distorch_config.get("high_precision_loras", True)
                            )
...
class CheckpointLoaderAdvancedDisTorch2MultiGPU:
        ...
        return {
            "required": {
                ...
                "clip_virtual_vram_gb": (
                    "FLOAT",
                    {"default": 2.0, "min": 0.0, "max": 128.0, "step": 0.1},
                ),
    ...
    def load_checkpoint(
        ...
        if eject_models:
            logger.mgpu_mm_log(
                "[EJECT_MODELS_SETUP] eject_models=True - marking all loaded models for eviction"
            )
            ejection_count = 0
            for i, lm in enumerate(mm.current_loaded_models):
                model_name = (
                    type(getattr(lm.model, "model", lm.model)).__name__
                    if lm.model
                    else "Unknown"
                )
                if hasattr(lm.model, "model") and lm.model.model is not None:
                    ...
                    logger.mgpu_mm_log(
                        f"[EJECT_MARKED] Model {i}: {model_name} (id=0x{id(lm):x}) → marked for eviction"
                    )
                    ejection_count += 1
                elif lm.model is not None:
                    lm.model._mgpu_unload_distorch_model = True
                    logger.mgpu_mm_log(
                        f"[EJECT_MARKED] Model {i}: {model_name} (direct patcher) → marked for eviction"
                    )
                    ejection_count += 1
            logger.mgpu_mm_log(
                f"[EJECT_MODELS_SETUP_COMPLETE] Marked {ejection_count} models for Comfy Core eviction during load_models_gpu"
            )
        ...
        if unet_virtual_vram_gb > 0:
            unet_vram_str = (
                f"{unet_compute_device};{unet_virtual_vram_gb};{unet_donor_device}"
            )
        elif unet_expert_mode_allocations:
            unet_vram_str = unet_compute_device
        unet_alloc = (
            f"{unet_expert_mode_allocations}#{unet_vram_str}"
            if unet_expert_mode_allocations or unet_vram_str
            else ""
        )
        ...
        if clip_virtual_vram_gb > 0:
            clip_vram_str = (
                f"{clip_compute_device};{clip_virtual_vram_gb};{clip_donor_device}"
            )
        elif clip_expert_mode_allocations:
            clip_vram_str = clip_compute_device
        clip_alloc = (
            f"{clip_expert_mode_allocations}#{clip_vram_str}"
            if clip_expert_mode_allocations or clip_vram_str
            else ""
        )
