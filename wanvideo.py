...
class WanVideoModelLoader:
        ...
        return {
            ...
            "optional": {
                ...
                "extra_model": (
                    ...
                    {
                        "default": None,
                        "tooltip": "Extra model to add to the main model, ie. VACE or MTV Crafter",
                    },
                ),
                "vace_model": (
                    "VACEPATH",
                    {
                        "default": None,
                        "tooltip": "Backward-compatible alias for extra_model",
                    },
                ),
                ...
                "multitalk_model": (
                    "MULTITALKMODEL",
                    {"default": None, "tooltip": "Multitalk model"},
                ),
                "fantasyportrait_model": (
                    "FANTASYPORTRAITMODEL",
                    {"default": None, "tooltip": "FantasyPortrait model"},
                ),
                "rms_norm_function": (
                    ["default", "pytorch"],
                    {
                        "default": "default",
                        "tooltip": "RMSNorm function to use, 'pytorch' is the new native torch RMSNorm, which is faster (when not using torch.compile mostly) but changes results slightly. 'default' is the original WanRMSNorm",
                    },
                ),
            },
        }
    ...
    def loadmodel(
        self, model, base_precision, compute_device, quantization, load_device, **kwargs
    ):
        ...
        if (
            kwargs.get("vram_management_args") is not None
            and kwargs.get("rms_norm_function") == "pytorch"
        ):
            logger.warning(
                "[MultiGPU WanVideoWrapper] PyTorch RMSNorm is incompatible with "
                "WanVideo VRAM management; using the offload-aware default RMSNorm."
            )
            kwargs["rms_norm_function"] = "default"
...
class WanVideoSampler:
            ...
            "optional": {
                ...
                "start_step": (
                    "INT",
                    {
                        ...
                        "max": 10000,
                        "step": 1,
                        "tooltip": "Start step for the sampling, 0 means full sampling, otherwise samples only from this step",
                    },
                ),
                "end_step": (
                    "INT",
                    {
                        ...
                        "max": 10000,
                        "step": 1,
                        "tooltip": "End step for the sampling, -1 means full sampling, otherwise samples only until this step",
                    },
                ),
                "add_noise_to_samples": (
                    ...
                    {
                        "default": False,
                        "tooltip": "Add noise to the samples before sampling, needed for video2video sampling when starting from clean video",
                    },
                ),
            },
        }
    ...
    def process(self, model, compute_device, **kwargs):
        ...
        multi_gpu_block_swap = (
            block_swap_args is not None and "swap_device" in block_swap_args
        )
        offload_device_to_be_patched = None
        if multi_gpu_block_swap:
            swap_label = block_swap_args.get("swap_device")
            logger.info(
                f"[MultiGPU WanVideoWrapper][WanVideoSamplerMultiGPU] block swap enabled, swap device: {swap_label}"
            )
...
class WanVideoTextEncode:
    ...
    def process(
        ...
        prompt_embeds_dict = original_encoder.process(
            positive_prompt,
            negative_prompt,
            text_encoder,
            force_offload,
...
class LoadWanVideoT5TextEncoder:
    ...
    def loadmodel(self, model_name, precision, device=None, quantization="disabled"):
        ...
        text_encoder = original_loader.loadmodel(
            model_name, precision, load_device, quantization
        )
...
class WanVideoTextEncodeCached:
        ...
        return {
            ...
            "optional": {
                "extender_args": (
                    ...
                    {
                        "tooltip": "Use this node to extend the prompt with additional text."
                    },
                ),
            },
        }
    ...
    def process(
        ...
        prompt_embeds_dict, negative_text_embeds, positive_prompt_out = (
            original_encoder.process(
                model_name,
                precision,
                positive_prompt,
                negative_prompt,
...
class WanVideoVAELoader:
    ...
    def loadmodel(
        self, model_name, load_device=None, precision="fp16", compile_args=None
    ):
        from . import set_current_device
    ...
    def INPUT_TYPES(s):
        ...
        return {
            ...
            "optional": {
                ...
                "parallel": (
                    "BOOLEAN",
                    {"default": False, "tooltip": "uses more memory but is faster"},
                ),
            },
        }
...
class WanVideoBlockSwap:
    @classmethod
    def INPUT_TYPES(s):
        base_inputs = copy.deepcopy(
            NODE_CLASS_MAPPINGS["WanVideoBlockSwap"].INPUT_TYPES()
        )
        devices = get_device_list()
    ...
    def INPUT_TYPES(s):
        return {
            ...
            "optional": {
                ...
                "add_cond_latents": (
                    "ADD_COND_LATENTS",
                    {"advanced": True, "tooltip": "Additional cond latents WIP"},
                ),
            },
        }
...
class WanVideoVACEEncode:
    ...
    def process(
        ...
        try:
            return original_encode.process(
                vae[0],
                width,
                height,
                num_frames,
            ...
            "required": {
                ...
                "tile_stride_y": (
                    "INT",
                    {
                        ...
                        "max": 2048,
                        "step": 32,
                        "tooltip": "Tile stride height in pixels, smaller values use less VRAM, may introduce more seams",
                    },
                ),
            },
            "optional": {
...
class Wav2VecModelLoader:
    ...
    def INPUT_TYPES(s):
        ...
        return {
            "required": {
                "model": (
                    folder_paths.get_filename_list("wav2vec2"),
                    {
                        "tooltip": "These models are loaded from the 'ComfyUI/models/wav2vec2' -folder",
                    },
                ),
                "base_precision": (["fp32", "bf16", "fp16"], {"default": "fp16"}),
                "load_device": (
